...
function startTimer() {
    ...
    if (!subject) {
        alert('Por favor, ingresa la materia o tema antes de iniciar.');
        return;
    }
...
function stopTimer() {
    ...
    const summaryText = `Materia: ${currentSubject} | Duración: ${formatMs(elapsedTime)}`;
    document.getElementById('modal-session-summary').innerText = summaryText;
    document.getElementById('notes-modal').classList.remove('hidden');
}
...
function renderSessionsList() {
    ...
    if (sessions.length === 0) {
        container.innerHTML = '<p style="color: var(--text-muted); text-align: center; padding: 20px;">No hay sesiones registradas aún.</p>';
        return;
    }
...
function confirmResetData() {
    if (sessions.length === 0) {
        alert("No hay datos guardados para restablecer.");
        return;
    }

    const confirmed = confirm("¿Estás seguro de que deseas eliminar todas las sesiones guardadas? Esta acción no se puede deshacer.");
...
>>>>>>> 1759a20c6f2208aa4210accbeeef871ad096b828
}